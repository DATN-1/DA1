...
export default function CreateBanner() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setMessage(data?.message || 'Tạo banner thành công');
      setForm(defaultForm);
    ...
    <div className="content-padding products-admin">
      ...
      <div className="products-card" style={{ maxWidth: '680px', margin: '0 auto', padding: '2rem', background: 'white', borderRadius: '1rem', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#1a1a1a', margin: 0 }}>Thêm Banner Mới</h2>
          <button
            type="button"
        ...
        <form className="products-form" onSubmit={handleSubmit}>
          ...
          <div style={{ marginTop: '1rem' }}>
            ...
            >
              {submitting ? 'Đang lưu...' : 'Thêm banner'}
            </button>
          </div>
        </form>
      </div>
