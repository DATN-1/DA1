...
export default function VerifyEmailPage() {
  ...
  const [verificationType, setVerificationType] = useState<'token' | 'code' | null>(null);
      ...
      if (token && emailParam) {
        ...
        try {
          ...
          if (!response.ok) {
            ...
            setMessage(data.message || 'Email đã được xác thực thành công. Tài khoản của bạn đã được kích hoạt. Chuyển hướng đến trang đăng nhập...');
        ...
        // Fallback to OTP code verification
        const emailFallback = params.get('email');
        if (emailFallback) {
          setEmail(emailFallback);
        }
        setVerificationType('code');
        setMessage('Nhập email và mã kích hoạt đã nhận được qua email.');
        setLoading(false);
      }
    };
      ...
      <div className="auth-page" style={{ padding: '2rem' }}>
        <div className="auth-container">
          <div className="auth-header">
            <h2 className="text-gradient">Xác Thực Email</h2>
          ...
          <div style={{ textAlign: 'center', padding: '2rem' }}>
            ...
            <p className={`auth-feedback ${message.includes('thành công') ? 'success' : message.includes('Đang') ? '' : 'error'}`}>
              {message}
            </p>
          </div>
          {!loading && (
