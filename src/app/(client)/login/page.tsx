    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (response?.error) {
                ...
                // Fetch the secure session generated by NextAuth
                const sessionResponse = await fetch('/api/auth/session');
                const sessionData = await sessionResponse.json();
                
                if (typeof window !== 'undefined' && sessionData?.user) {
                    window.localStorage.setItem('currentUser', JSON.stringify(sessionData.user));
                    window.sessionStorage.setItem('authMessage', 'Đăng nhập thành công');
                    window.dispatchEvent(new CustomEvent('authChanged', { detail: { user: sessionData.user } }));
                }
                setFeedback('Đăng nhập thành công. Đang chuyển hướng...');
                window.setTimeout(() => {
                    router.push('/');
                    router.refresh(); // Refresh Next.js cache to pick up cookies server-side
                }, 600);
            }
        } catch (error) {
