...
export async function createEmailVerificationToken(
    ...
    const [result] = await pool.query(
        `INSERT INTO user_email_tokens (user_id, email, token, expires_at, purpose)
         VALUES (?, ?, ?, ?, 'verify-account')`,
        [userId, email, token, expiresAt]
    );
    return result;
}

export async function findValidEmailToken(token: string, email: string) {
    await ensureVerificationTokenTable();
    const [rows] = await pool.query(
        'SELECT * FROM user_email_tokens WHERE token = ? AND email = ? AND expires_at >= NOW() LIMIT 1',
        [token, email]
    );
...
export async function deleteEmailToken(token: string) {
    await ensureVerificationTokenTable();
    const [result] = await pool.query(
        'DELETE FROM user_email_tokens WHERE token = ?',
        [token]
    );
    return result;
}